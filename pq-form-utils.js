...
function startFormUtils(sfuForm, sfuCountry, sfuStateprov, sfuMarket, sfuSubmarket, sfuEmail, sfuFirstname){
	...
	$('#'+sfuMarket).change(function(e){							// add country check function to change on dropdown
		checkMarketSubmarket('change',sfuMarket,sfuSubmarket);
	});
}

function checkCountryState (mode,sfuCountry,sfuStateprov){					// Function to check the value of the "Country" field and set the "State/Province" field accordingly
	var currentState = $('#'+sfuStateprov+' option:selected').val();
	$('#'+sfuStateprov).empty();								// clear existing options from state/province dropdown
	var currentCountry = $("#"+sfuCountry).val();						// check country field
...
function checkMarketSubmarket (mode,sfuMarket, sfuSubmarket){					// Fucntion to check the value of the "Market" field and set the "Submarket" field accordingly
...
function clearIdentity(sfuForm,sfuEmail){							// Called to clear the contact form, including cookie and hidden recipient id field
...
function initFormIdentityLink(sfuForm,sfuFirstname,sfuEmail){							// Initialize the identity clearing framework if applicable
	//debugger;
	if ( $( "[name='RECIPIENT_ID_*']" ).length ) {
		...
		$( '#clear-identity-action' ).click(function(e){					// clearForm function to click event
